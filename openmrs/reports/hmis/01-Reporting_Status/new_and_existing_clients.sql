...
FROM
  (SELECT DISTINCT patient.patient_id AS patient_id,
   ...
   FROM visit
     ...
     -- JOIN visit_type vt ON visit.visit_type_id = vt.visit_type_id AND vt.name in ( 'IPD','Emergency','OPD','Proxy','PHARMACY','LAB VISIT')
     RIGHT OUTER JOIN reporting_age_group_new AS observed_age_group ON
