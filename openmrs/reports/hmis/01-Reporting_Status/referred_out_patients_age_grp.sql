...
FROM
    ...
    RIGHT JOIN reporting_age_group_new AS observed_age_group ON DATE(client_visit.date_stopped) BETWEEN (DATE_ADD(client_visit.birthdate, INTERVAL observed_age_group.min_years YEAR)) AND (DATE_ADD(client_visit.birthdate, INTERVAL observed_age_group.max_years YEAR))
    WHERE
