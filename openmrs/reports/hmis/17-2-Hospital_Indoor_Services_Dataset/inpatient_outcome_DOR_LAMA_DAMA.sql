...
FROM
    ...
    ORDER BY answer_name DESC) first_answers
     ...
     (SELECT 'M' AS gender UNION SELECT 'F' AS gender) gender
        INNER JOIN
    reporting_age_group_new rag ON rag.report_group_name = 'Inpatient'
        LEFT OUTER JOIN
    (SELECT 
	...
	INNER JOIN person on o1.person_id = person.person_id
     INNER JOIN reporting_age_group_new on cast(o1.obs_datetime AS DATE) BETWEEN (DATE_ADD(DATE_ADD(person.birthdate, INTERVAL reporting_age_group_new.min_years YEAR), INTERVAL reporting_age_group_new.min_days DAY))
...
ORDER BY FIELD(rag.name, '≤ 7 Days','8 - 28 Days','29 Days ‐ 1 Year','01 ‐ 04 Years','05 ‐ 14 years','15 ‐ 19 Years','20 ‐ 29 Years','30 ‐ 39 Years','40 ‐ 49 Years','50 ‐ 59 Years','60 ‐ 69 Years','≥ 70 Years'), gender.gender ;
